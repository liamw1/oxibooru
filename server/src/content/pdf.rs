...
pub fn preview_image(file_path: &Path) -> ApiResult<DynamicImage> {
    let pdf = {
    ...
    let dimensions = PdfRenderDimensions::from_page(page)?;
    let pixmap =
...
impl PdfRenderDimensions {
    ...
    fn from_page(page: &Page<'_>) -> ApiResult<Self> {
        // These values are not for rejecting large PDFs but for clamping the size of the preview image.
        const MAX_PDF_PREVIEW_WIDTH: u16 = 1200;
        const MAX_PDF_PREVIEW_HEIGHT: u16 = 1200;

        let (width, height) = page.render_dimensions();

        // Find the min ratio to scale down the image while maintaining aspect ratio
        let scale = {
            let max_width = f32::from(MAX_PDF_PREVIEW_WIDTH);
            let max_height = f32::from(MAX_PDF_PREVIEW_HEIGHT);
