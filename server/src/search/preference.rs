...
impl<'a> Preferences<'a> {
    pub fn new(config: &'a Config, client: Client) -> Self {
        Self { client, config }
    }

    pub fn is_empty(&self) -> bool {
        self.client.rank != UserRank::Anonymous || self.config.anonymous_preferences.is_empty()
    }

    pub fn hidden_posts<C>(&self, post_id_column: C) -> Option<BoxedQuery>
    where
        C: Expression<SqlType = BigInt> + QueryFragment<Pg> + Send + 'static,
    {
        if self.client.rank != UserRank::Anonymous {
            return None;
        }
        let preferences = &self.config.anonymous_preferences;
