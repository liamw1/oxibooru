...
fn array_diff(old: &Vec<Value>, new: &Vec<Value>) -> Option<Value> {
    let added: Vec<_> = new.iter().filter(|item| !old.contains(*item)).cloned().collect();
...
fn object_diff(old: &Map<String, Value>, new: &Map<String, Value>) -> Option<Value> {
    let mut diff = Map::new();

    // Check for keys present in old object
    for (key, old_value) in old {
        let new_value = match new.get(key) {
            Some(new_value) => new_value,
            None => {
                // Property deleted
                let change_type = ("type".into(), "deleted property".into());
                let value_deleted = ("value".into(), old_value.clone());
    ...
    for (key, new_value) in new.iter().filter(|(key, _)| !old.contains_key(*key)) {
        let change_type = ("type".into(), "added_property".into());
        let value_added = ("value".into(), new_value.clone());
